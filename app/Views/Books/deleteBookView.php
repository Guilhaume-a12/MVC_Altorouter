    ...
    <div class="row mt-2">
        <div class="col-6">
            <form action="<?=$this->router->generate('books-delete-validate',["id" => $book->getId()])?>" method="POST" class="">
                <button class="btn btn-danger w-100" name="btnDel">DELETE</button>
                <input type="hidden" name="token" value="<?=$_SESSION['token']?>">
            </form>
        </div>
        <div class="col-6">
            <a href="<?=$this->router->generate('books')?>" class="btn btn-success d-block">CANCEL</a>
        </div>
    </div>
</div>

<?php
$titre = "Delete ".$book->getTitle()." ?";
